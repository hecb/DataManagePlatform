		...
		<div class="col-xs-12">
			<form class="form-horizontal" id="permForm" action="${staticPath }/perm/addPerm" method="post">
				<div class="form-group"> 
					<input type="hidden" name="id" id="id" value="${prem.id }"/>
					...
					<div class="col-md-4"> 
						<select class="form-control" name="type" id="type">
									<option value="">&nbsp;</option>
										...
										<option value="menu"  <c:if test="${prem.type eq  'menu'}">selected='selected'</c:if> >菜单</option>
									
									<option value="btn" <c:if test="${prem.type eq  'btn'}">selected='selected'</c:if> >按钮</option>
						</select>
					</div>
			 	</div>
